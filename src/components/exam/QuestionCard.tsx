...
import { Question, SECTION_LABELS, SECTION_COLORS, Section } from '@/types';
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
      <div className="flex items-center justify-between mb-3">
        ...
        <span className={`text-xs px-2 py-1 rounded-full font-medium ${SECTION_COLORS[question.topic as Section]}`}>
          {SECTION_LABELS[question.topic as Section]}
        </span>
      </div>

      {question.passageContext && (
        <div className="mb-4 p-4 bg-gray-50 rounded-lg border border-gray-200 text-sm text-gray-700 leading-relaxed">
          <p className="text-xs font-semibold text-gray-400 mb-2 uppercase tracking-wide">Passage</p>
          <p className="whitespace-pre-line">{question.passageContext}</p>
        </div>
      )}
