...
import { Question, Section, TopicWeight } from '@/types';
...
function rowToQuestion(row: Record<string, unknown>): Question {
  return {
    ...
    passageContext: row.passage_context as string | null,
...
export async function getQuestionsByTopic(topic: Section, limit = 50): Promise<Question[]> {
  const db = getDb();
...
export async function getQuestionsSampled(
  ...
  const db = getDb();
  const topicCounts = new Map<Section, number>();
...
export async function insertQuestion(q: Omit<Question, 'id' | 'createdAt'>): Promise<number> {
  ...
  const result = await db.execute({
    sql: `INSERT INTO questions
      (source, source_year, topic, difficulty, question_text, option_a, option_b, option_c, option_d, correct_answer, explanation, passage_context, page_number)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    args: [
      q.source, q.sourceYear, q.topic, q.difficulty,
      q.questionText, q.optionA, q.optionB, q.optionC, q.optionD,
      q.correctAnswer, q.explanation, q.passageContext, q.pageNumber,
    ],
  });
...
export async function getQuestionsForTopics(topics: Section[], count: number): Promise<Question[]> {
