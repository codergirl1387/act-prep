...
import { PerformanceRecord, Section, TopicBreakdown } from '@/types';
...
export async function getTopicAccuracySummary(): Promise<{ topic: Section; accuracy: number; total: number }[]> {
  const db = getDb();
