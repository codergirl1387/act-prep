import { Section, TopicWeight, PerformanceRecord, SECTIONS } from '@/types';
...
export function computeTopicWeights(
  ...
  const topicStats: Map<Section, { weightedCorrect: number; weightedTotal: number }> =
    new Map(SECTIONS.map((t) => [t, { weightedCorrect: 0, weightedTotal: 0 }]));
  ...
  const accuracyByTopic = new Map<Section, number>();
  ...
  const needScores = new Map<Section, number>();
  ...
  const weights: TopicWeight[] = SECTIONS.map((topic) => ({
    topic,
...
export function sampleTopicsByWeight(weights: TopicWeight[], count: number): Section[] {
  const selected: Section[] = [];
  const sorted = [...weights].sort((a, b) => a.weight - b.weight);
  const cumulative: { topic: Section; threshold: number }[] = [];
  let sum = 0;
  for (const w of sorted) {
    sum += w.weight;
    cumulative.push({ topic: w.topic as Section, threshold: sum });
  }
