...
export async function POST(req: NextRequest) {
  ...
  for (const ans of answers) {
    ...
    processedAnswers.push({ questionId: ans.questionId, selectedAnswer: ans.selectedAnswer, isCorrect });
  ...
  await recordSessionPerformance(topicBreakdown, todayString(), 'exam');

  const actScores = computeACTScores(topicBreakdown);
